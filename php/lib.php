<?php
/**
 * TORTINMANG.UZ PHP Lite — yordamchi funksiyalar (v2)
 */
...
function journal_load($dir) {
    $d = json_decode(@file_get_contents($dir . '/journal.json'), true);
    return is_array($d) ? $d : [];
}

function journal_stats($dir, $symbol = null) {
    $j = journal_load($dir);
