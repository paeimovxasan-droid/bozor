...
if (empty($cfg['DB_NAME'])) {
    row(true, 'MySQL (sozlanmagan — json fayl rejimi)');
} else {
    $pdo = db_conn($cfg);
    if ($pdo) {
        try {
            db_schema($pdo);
            row(true, 'MySQL ulanish + jadvallar', $cfg['DB_NAME']);
        } catch (Exception $e) {
            row(false, 'MySQL jadval yaratish', $e->getMessage());
        }
    } else {
        row(false, 'MySQL ulanish', 'host/user/parolni tekshiring');
    }
}
