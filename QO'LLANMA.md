# 📖 TORTINMANG.UZ — Libertex Edition | To'liq O'rnatish va Ishlatish Qo'llanmasi
...
```bash
# Git orqali
git clone https://github.com/paeimovxasan-droid/bozor.git
cd bozor

# Libertex branch (agar kerak bo'lsa)
git checkout arena/01a09db7-bozor
...
# ─── Fokus bozorlar va signal sezgirligi ─────────────────────
# Asosiy savdo bozorlari (default: XAUUSD, ETHUSD) va tahlil sikli
FOCUS_MARKETS=XAUUSD,ETHUSD
CYCLE_SECONDS=60
SCAN_ALL_MARKETS=false
# Ko'proq signal uchun (ehtiyotkorlik bilan!!): MIN_CONFIDENCE=40, MIN_ADX=18, SIGNAL_STRENGTH=12
MIN_CONFIDENCE=45
MIN_ADX=20
SIGNAL_STRENGTH=14
...
# ─── Database (ixtiyoriy) ────────────────────────────────────
DB_URL=postgresql://ultra:ultra123@localhost:5432/tortinmang_uz
...
2. `/newbot` → nom: `TORTINMANG.UZ Bot` → username: `tortinmang_uz_bot`
...
## 8. DeepSeek AI — yagona AI provayder

TORTINMANG.UZ barcha AI tahlillarida **faqat DeepSeek** API dan foydalanadi
(rasmiy docs: https://api-docs.deepseek.com).

### API key olish

1. https://platform.deepseek.com → Ro'yxatdan o'tish (to'lov balansini to'ldirish: **Top up**)
2. **API Keys** → **Create new API key** → `sk-...` ni nusxalash
3. `.env` ga yozish: `DEEPSEEK_API_KEY=sk-...`

### ⚠️ MUHIM — model nomlari (2026-yildan)

Eski `deepseek-chat` va `deepseek-reasoner` model nomlari **2026-07-24 da
to'liq o'chirilgan** — ular endi umuman ishlamaydi ("Model Not Exist" xatosi).
Hozirgi to'g'ri modellar:

| Model | Tavsif | Tavsiya |
|-------|--------|---------|
| `deepseek-flash` | DeepSeek-V4.1-Flash — arzon, tez | ✅ Bot uchun asosiy |
| `deepseek-v4-pro` | Kuchli reasoning, qimmatroq | Chuqur tahlil kerak bo'lsa |

Tortinmang buni avtomatik hal qiladi: `.env` da eski nom yozilgan bo'lsa ham
u `deepseek-flash` ga almashtiriladi.
...
# Thinking rejimi: none=tez/arzon (tavsiya) | low | high | max
...
> 💡 DeepSeek V4 da "thinking" rejimi default **yoqilgan** — bu har bir javobni
> sekin va qimmat qiladi. Shuning uchun bot har bir so'rovda aniq
> `reasoning_effort=none` yuboradi (kerak bo'lsa `.env` da o'zgartiring).

### Tekshirish

```bash
python test_deepseek.py        # alohida DeepSeek testi
python run.py --test           # to'liq tizim testi
```

Xato kodlari (bot sizga aniq sababini ko'rsatadi):
- **401 Authentication Fails** — kalit noto'g'ri, `.env` ni tekshiring
- **402 Insufficient Balance** — platformada balans to'ldiring
- **404 Model Not Exist** — model nomi eskirgan (yuqoriga qarang)
- **429 Rate Limit** — limit oshdi, bot avtomatik qayta urinadi

Bo'lmasa ham bot ishlaydi, faqat `/ask` va AI tahlil o'chadi.
...
- Savdolar `logs/trades.log` va `tortinmang_state.json` da saqlanadi
...
```
🚀 TORTINMANG.UZ — Libertex Edition
...
- `Ctrl+C` → state `tortinmang_state.json` ga saqlanadi → keyingi safar davom etadi
- Telegram: `/stop`
...
3. Action: `python C:\path\to\bozor\run.py --bot`
...
| `tortinmang_state.json` | State (cycle, balance) | ❌ Yo'q |
...
- GitHub repo: https://github.com/paeimovxasan-droid/bozor
...
**Omadli savdo!** 💰 Libertex MT5 + TORTINMANG.UZ = $10 → $1M
