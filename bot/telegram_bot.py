"""
TORTINMANG.UZ — Enhanced Telegram Bot
...
"""
    ...
    async def _cmd_status(self, msg, args):
        ...
        if hasattr(o, "tier_engine"):
            tier = o.tier_engine.current().name + " (v2)"
        else:
    ...
    async def _cmd_closeall(self, msg, args):
        ...
        # v2 bot: statistika/learning bilan to'g'ri yopish yo'lini ishlatamiz
        if hasattr(o, "_v2_close_position"):
            for pos in positions:
                try:
                    if await o._v2_close_position(pos, "MANUAL"):
                        count += 1
                except Exception as e:
                    logger.debug(f"v2 close {pos.get('symbol')}: {e}")
        ...
        for pos in positions:
            try:
                ...
                if pos.get("is_paper"):
                    ...
                    o.execution.close_position(ticket)
                    count += 1
            except Exception as e:
    ...
    async def send_startup_ultra(self, balance: float, server: str, tier: str, active_markets: int):
        ...
        msg = f"""
🚀 <b>TORTINMANG.UZ — ISHGA TUSHDI!</b>
...
"""
    ...
    async def send_daily_report(self, balance: float, equity: float, growth_pct: float,
        ...
        msg = f"""
📊 <b>TORTINMANG.UZ — KUNLIK HISOBOT</b>
...
🤖 TORTINMANG.UZ | Multi-Market System
"""
        await self.send(msg)
