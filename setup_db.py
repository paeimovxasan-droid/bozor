...
def setup_postgres():
    ...
    try:
        ...
        # Database yaratish
        try:
            cur.execute("CREATE DATABASE tortinmang_uz OWNER ultra;")
            print("[OK] Database 'tortinmang_uz' yaratildi")
        except Exception as e:
            if "already exists" in str(e):
        ...
        cur.execute("GRANT ALL PRIVILEGES ON DATABASE tortinmang_uz TO ultra;")
...
def setup_redis():
    try:
        ...
        r.ping()
        r.set("tortinmang_uz:status", "active")
        r.set("tortinmang_uz:version", "1.0")
...
def test_connection():
    ...
    db_url = os.getenv("DB_URL", "postgresql://ultra:ultra_secure_2025@localhost:5432/tortinmang_uz")
    try:
        import psycopg2
        # URL ni parse qilish
        conn = psycopg2.connect(
...
if __name__ == "__main__":
    print("=" * 50)
    print("  TORTINMANG.UZ — Database Setup")
    print("=" * 50)
