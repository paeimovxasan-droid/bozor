"""
TORTINMANG.UZ — Multi-Market Orchestrator
...
class UltraOrchestrator:
    ...
    def __init__(self):
        ...
        self._dogon_count: dict = {}      # symbol → nechta dogon qilingan
        self._dogon_last: dict = {}       # symbol → oxirgi dogon vaqti (cooldown)
        self._partial_done: set = set()   # partial TP qilingan ticketlar
        self._sym_stats: dict = {}        # symbol → {trades, wins, profit} (fokus optimallash)
        self._last_report_day = None      # kunlik AI hisobot uchun

        # State fayli joyi (script yonida)
        _base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self._state_file = os.path.join(_base, "tortinmang_state.json")
        # Migratsiya: eski nomdagi state bo'lsa — statistika yo'qolmasin
        _old_state = os.path.join(_base, "goldai_state.json")
        if not os.path.exists(self._state_file) and os.path.exists(_old_state):
            try:
                os.replace(_old_state, self._state_file)
            except Exception:
                self._state_file = _old_state

    # ─── INIT ─────────────────────────────────────────────────────

    async def initialize(self) -> bool:
        logger.info("=" * 65)
        logger.info("🚀 TORTINMANG.UZ — Libertex Edition")
        ...
        if config.focus.scan_all:
            logger.info(f"📊 Aktiv bozorlar ({len(active)}): {', '.join(active[:6])}...")
        else:
            logger.info(f"🎯 FOKUS bozorlar: {', '.join(active)} (FOCUS_MARKETS)")
        logger.info(f"⏱️ Tahlil sikli: har {config.focus.cycle_seconds} soniya")
    ...
    def _get_active_symbols(self, balance: float) -> list:
        """
        Rejimga qarab aktiv symbollar.
        FOCUS_MARKETS berilgan bo'lsa (default: XAUUSD,ETHUSD) — faqat shular
        tahlil qilinadi va savdo qilinadi. SCAN_ALL_MARKETS=true da esa
        balans tier'idagi barcha bozorlar skanerlanadi.
        """
        if not config.focus.scan_all:
            symbols = [s for s in config.focus.symbols if s in MARKETS] \
                or config.get_active_symbols(balance)
        else:
            symbols = config.get_active_symbols(balance)

        if self._mode == self.MODE_BINANCE:
            # Binance fallback — faqat crypto
            try:
                from engines.binance_executor import SYMBOL_MAP
                return [s for s in symbols if s in SYMBOL_MAP]
            except Exception:
                return [s for s in symbols if MARKETS.get(s, {}).get("type") == "crypto"]
        # Libertex MT5
        return symbols
    ...
    async def run_cycle(self):
        ...
        try:
            ...
            if scan.total_signals == 0:
                ...
                # 6b. Smart Dogon — zarar ko'rayotgan pozitsiya + yangi tasdiq signal
            ...
            for cp in closed_papers:
                ...
                self._record_stat(cp["symbol"], cp["profit"])
    ...
    async def _process_signal(self, signal: MarketSignal, account: dict,
        ...
        # Spread himoyasi — yangilik paytida spread kengaysa savdo ochilmaydi
        if not self._spread_ok(signal.symbol):
            return
    ...
    async def _execute_trade(self, decision: TradingDecision):
        ...
        comment = f"Tortinmang {decision.market_type[:3].upper()} {decision.confidence:.0f}%"
        ...
        if result.success:
            ...
            trade_log.log_open(
                ...
                lot=decision.lot,
                entry=getattr(result, "price", 0) or decision.entry,
    ...
    async def _dogon_check(self, scan, account: dict):
        """
        Smart Dogon: ochiq pozitsiya zararda bo'lsa VA skaner shu symbol bo'yicha
        AYNI yo'nalishda yangi, kuchliroq signal topgan bo'lsa — bitta qo'shimcha
        pozitsiya ochadi. Nazoratsiz martingale emas — qat'iy limitlar:
          • DOGON_MAX (default 1)         — har pozitsiyaga necha dogon
          • DOGON_LOT_MULT (default 1.0)  — lot ko'paytiruvchisi (xavfsiz)
          • DOGON_MIN_CONF (default 50)   — dogon signali kuchliroq bo'lishi kerak
          • DOGON_MAX_LOSS_PCT (2%)       — katta zararda dogon QILINMAYDI
          • DOGON_COOLDOWN (30 min)       — bir symbolga tez-tez dogon yo'q
        """
        if not config.dogon.enabled or not scan.best_signals:
            return
        ...
        # Ochiq pozitsiyasi yo'q symbollar uchun hisoblagichni reset qilish
        open_syms = {p["symbol"] for p in positions}
        for sym in list(self._dogon_count):
            if sym not in open_syms:
                self._dogon_count[sym] = 0

        for pos in positions:
            symbol = pos.get("symbol")
            side = pos.get("type") or pos.get("side")
            profit = float(pos.get("profit") or 0)
            if profit >= 0 or side not in ("BUY", "SELL"):
                continue

            # Spread himoyasi + runaway himoyasi: zarar limitdan oshgan bo'lsa dogon yo'q
            if not self._spread_ok(symbol):
                continue
            loss_pct = abs(profit) / max(balance, 1.0) * 100
            if loss_pct > config.dogon.max_loss_pct:
                continue
            if self._dogon_count.get(symbol, 0) >= config.dogon.max_per_symbol:
                continue
            last = self._dogon_last.get(symbol, 0)
            if time.time() - last < config.dogon.cooldown_seconds:
                continue

            # Ayni symbol + ayni yo'nalish + kuchliroq ishonch
            signal = next(
                (s for s in scan.best_signals
                 if s.symbol == symbol and s.signal == side
                 and s.confidence >= config.dogon.min_confidence),
                None,
            )
            if signal is None:
                continue

            base_lot = float(pos.get("volume") or pos.get("lot") or 0)
            if base_lot <= 0:
                continue
            dogon_lot = round(base_lot * config.dogon.lot_multiplier, 2)

            sl_dist = abs(signal.entry - signal.stop_loss) or 1e-10
            rr = abs(signal.take_profit - signal.entry) / sl_dist
            n = self._dogon_count.get(symbol, 0) + 1
            ...
            decision = TradingDecision(
                ...
                rr_ratio=round(rr, 2),
                tier=config.risk.get_for_balance(balance)["tier"],
                reason=f"🔁 DOGON #{n} (zarar {loss_pct:.1f}%) | {signal.reason}",
                whale_involved=False,
                timestamp=datetime.utcnow().isoformat(),
            )

            self._dogon_count[symbol] = n
            self._dogon_last[symbol] = time.time()
            logger.info(
                f"🔁 DOGON {symbol} {side} #{n}: zarar {loss_pct:.1f}% | "
                f"yangi signal {signal.confidence:.1f}% | lot {dogon_lot}"
            )
            await self._execute_trade(decision)
    ...
    async def _manage_positions(self, balance: float):
        ...
        for pos in positions:
            ...
            # ── PARTIAL TP: narx 1R ga yetganda 50% ni yopib, foydani qulflash ──
            if ticket not in self._partial_done:
                sl0 = float(pos.get("sl") or pos.get("stop_loss") or 0)
                r_dist = abs(open_price - sl0)
                if r_dist > 0 and side in ("BUY", "SELL"):
                    gain = (current - open_price) if side == "BUY" else (open_price - current)
                    if gain >= r_dist:
                        self._partial_done.add(ticket)
                        try:
                            if self.paper.is_paper_mode(balance):
                                pc = self.paper.partial_close(ticket, 0.5)
                            elif self._mode == self.MODE_MT5 and self.execution:
                                pc = self.execution.partial_close(ticket, 0.5)
                            else:
                                pc = None
                            if pc and getattr(pc, "success", False):
                                logger.info(f"💰 Partial TP: #{ticket} 50% yopildi (1R) — foyda qulflandi")
                                await self.telegram.send(
                                    f"💰 <b>Partial TP</b> #{ticket} {pos['symbol']}\n"
                                    f"50% hajm 1R da yopildi — foyda qulflandi, SL breakeven ga."
                                )
                        except Exception as e:
                            logger.debug(f"Partial TP xato #{ticket}: {e}")

            # Break-even va trailing — faqat real MT5 pozitsiyalari uchun
            # (paper pozitsiyalarni update_prices() boshqaradi, Binance da AttributeError bo'lardi)
            is_paper = bool(pos.get("is_paper")) or self.paper.is_paper_mode(balance)
            if self._mode == self.MODE_MT5 and not is_paper:
                if profit > 0:
    ...
    # ─── YORDAMCHI: SPREAD / STATISTIKA / KUNLIK HISOBOT ──────────

    def _spread_ok(self, symbol: str) -> bool:
        """
        Spread himoyasi: spread config.risk.max_spread_pct dan katta bo'lsa
        (yangilik paytida) savdo ochilmaydi — slippage dan himoya.
        """
        try:
            ...
            if spread_pct > config.risk.max_spread_pct:
                logger.info(
                    f"🚫 {symbol} spread katta: {spread_pct:.3f}% > "
                    f"{config.risk.max_spread_pct}% — savdo vaqtincha blok"
                )
                return False
            return True
        except Exception:
    ...
    def _record_stat(self, symbol: str, profit: float):
        """Har yopilgan savdo bo'yicha symbol statistikasi (fokus optimallash)"""
        st = self._sym_stats.setdefault(
            symbol, {"trades": 0, "wins": 0, "profit": 0.0}
        )
        st["trades"] += 1
        if profit > 0:
            st["wins"] += 1
    ...
    def _win_rate(self, symbol: str) -> float:
        st = self._sym_stats.get(symbol)
        if not st or st["trades"] < 3:
            return -1.0  # ma'lumot yetarli emas
        return st["wins"] / st["trades"] * 100

    async def _daily_report(self, account: dict):
        """Kunlik AI hisobot — DeepSeek yozadi, Telegram ga boradi"""
        try:
            balance = float(account.get("balance") or 0)
            stats = self.risk.get_statistics(balance)
            sym_lines = []
            for sym, st in sorted(self._sym_stats.items(),
                                  key=lambda kv: kv[1]["profit"], reverse=True):
                wr = self._win_rate(sym)
                wr_s = f"{wr:.0f}%" if wr >= 0 else "—"
                sym_lines.append(
                    f"{sym}: {st['trades']} savdo, win-rate {wr_s}, P/L {st['profit']:+.2f}"
                )
            context = {
                ...
                "focus_symbols": self._get_active_symbols(balance),
                "by_symbol": sym_lines or ["Hali yopilgan savdo yo'q"],
                "dogon_totals": dict(self._dogon_count),
            }
            report = await self.ai.daily_report(context)
            if report:
                await self.telegram.send(f"📊 <b>KUNLIK AI HISOBOT</b>\n\n{report}")
                logger.info("📊 Kunlik AI hisobot yuborildi")
        except Exception as e:
            logger.debug(f"Kunlik hisobot xato: {e}")
        ...
        for ticket, info in list(self._open_ticket_map.items()):
            if ticket not in current_tickets:
                ...
                if self._mode == self.MODE_BINANCE:
                    ...
                    self._record_stat(info["symbol"], profit)
    ...
    async def start(self):
        ...
        while self._running:
            try:
                ...
                if (self._mode == self.MODE_BINANCE and self.binance_exec
                    ...
                    try:
                        ...
                        for c in closed:
                            ...
                            self._record_stat(c["symbol"], c["profit"])
                ...
                # Har CYCLE_SECONDS da to'liq bozor siklini ishlatish
                # (default 60s = har daqiqa tahlil; .env da o'zgartiriladi)
                if now - _last_cycle_time >= config.focus.cycle_seconds:
                    _last_cycle_time = now
                    await self.run_cycle()

                    # Kunlik AI hisobot — kun almashganda Telegram ga
                    today = datetime.utcnow().date().isoformat()
                    if self._last_report_day is None:
                        self._last_report_day = today
                    elif self._last_report_day != today:
                        self._last_report_day = today
                        await self._daily_report(await self._get_account())
        ...
        logger.info("✅ TORTINMANG.UZ to'xtatildi")
    ...
    def _load_state(self):
        ...
        try:
            ...
            self._sym_stats = state.get("sym_stats", {}) or {}
            if state.get("last_report_day"):
                self._last_report_day = state["last_report_day"]
