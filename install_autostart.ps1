# ============================================================
# TORTINMANG.UZ — Windows Autostart O'rnatuvchi
...
$TASK_NAME = "TORTINMANG_Bot"
$BOT_DIR   = Split-Path -Parent $MyInvocation.MyCommand.Path
$PS_SCRIPT = Join-Path $BOT_DIR "start_tortinmang.ps1"
...
Write-Host "  TORTINMANG.UZ — Autostart O'rnatuvchi" -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $PS_SCRIPT)) {
    Write-Host "[!!] start_tortinmang.ps1 topilmadi: $PS_SCRIPT" -ForegroundColor Red
    pause
    exit 1
}
...
Register-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "TORTINMANG.UZ 24/7 Trading Bot — `$10 => `$1,000,000 Capital Growth" `
    -Force | Out-Null
