...
async def lifespan(app: FastAPI):
    global orchestrator
    # Agar run.py bot orchestratorni oldindan injekt qilgan bo'lsa,
    # ikkinchi nusxa yaratilmaydi (qo'shaloq MT5-login oldini oladi).
    if orchestrator is None:
        orchestrator = UltraOrchestrator()
        await orchestrator.initialize()  # async metod — await majburiy!
    yield
    if orchestrator:
        orchestrator.stop()
...
app = FastAPI(
    title="TORTINMANG.UZ — Libertex Edition",
...
@app.get("/")
async def root():
    return {"name": "TORTINMANG.UZ", "version": "2.0.0", "markets": len(MARKETS)}
