"""
TORTINMANG.UZ — Paper Trading (Demo Simulyatsiya)
...
class PaperTradingEngine:
    ...
    def _contract_size(self, symbol: str) -> float:
        sizes = {
            ...
            "XAUUSD": 100.0, "XAGUSD": 5000.0,   # MT5 kontrakt hajmlari
        }
        return sizes.get(symbol, 1.0)

    def partial_close(self, ticket: int, fraction: float = 0.5) -> PaperResult:
        """Pozitsiyaning 50% ini 'yopish (PartialPaper TP uchun)"""
        pos = self._positions.get(ticket)
        if pos is None or pos.closed:
            return PaperResult(success=False, error="Topilmadi")
        half_lot = pos.lot * fraction
        if half_lot <= 0:
            return PaperResult(success=False, error="Hajm juda kichik")
        pos.lot -= half_lot
        logger.info(f"📝 PAPER partial close #{ticket}: 50% yopildi, qolgan lot {pos.lot}")
        return PaperResult(success=True, ticket=ticket)
