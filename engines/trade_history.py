"""
TORTINMANG.UZ — Trade History Engine
...
# DB FAQAT .env da DB_URL berilgan bo'lsa yoqiladi — aks holda xotira rejimi.
# PostgreSQL o'rnatilmagan bo'lsa ham bot to'liq ishlaydi (savdolar logs/ da).
...
class TradeHistory:
    ...
    async def connect(self) -> bool:
        if asyncpg is None:
            ...
            return False
        if not DB_URL:
            logger.info("ℹ️ Trade History: DB_URL sozlanmagan — xotira rejimi (DB ixtiyoriy)")
            self._connected = False
            return False
        try:
