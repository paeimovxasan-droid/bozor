"""
TORTINMANG.UZ — Libertex (ForexClub) Execution Engine
...
class ExecutionEngine:
    ...
    def buy(self, symbol: str, lot: float, sl: float, tp: float, comment: str = "Tortinmang") -> TradeResult:
        return self._send_order(symbol, mt5.ORDER_TYPE_BUY, lot, sl, tp, comment)

    def sell(self, symbol: str, lot: float, sl: float, tp: float, comment: str = "Tortinmang") -> TradeResult:
    ...
    def _send_order(self, symbol: str, order_type: int, lot: float, sl: float, tp: float, comment: str) -> TradeResult:
        ...
        # Eslatma: yangi MT5 buildlarida SymbolInfo da `trade_allowed` atributi
        # YO'Q — uning o'rniga `trade_mode` ishlatiladi (SYMBOL_TRADE_MODE_FULL=4).
        # Ikkalasini ham xavfsiz tekshiramiz (eski/yangi terminal uchun).
        trade_mode = getattr(info, "trade_mode", None)
        trade_ok = (
            getattr(info, "trade_allowed", None)
            if getattr(info, "trade_allowed", None) is not None
            else (trade_mode is None or trade_mode == getattr(mt5, "SYMBOL_TRADE_MODE_FULL", 4))
        )
        if not trade_ok:
            logger.error(f"❌ {symbol}: savdo rejimi yopiq (trade_mode={trade_mode}) — bu instrumentda savdo ruxsat etilmagan")
            return TradeResult(success=False, error=f"{symbol}: savdo ruxsat etilmagan (trade_mode={trade_mode})")
    ...
    def partial_close(self, ticket: int, fraction: float = 0.5) -> TradeResult:
        """Pozitsiyaning bir qismini bozorda yopish (Partial TP uchun)"""
        positions = mt5.positions_get(ticket=ticket)
        if not positions:
            return TradeResult(success=False, error="Topilmadi")
        pos = positions[0]
        ...
        if info is None or tick is None:
            return TradeResult(success=False, error="Tick/symbol yo'q")

        vol = pos.volume * fraction
        vol = max(info.volume_min or 0.01, round(vol / (info.volume_step or 0.01)) * (info.volume_step or 0.01))
        vol = round(vol, 2)
        if vol >= pos.volume:
            return TradeResult(success=False, error="Hajm juda kichik — partial bo'lmaydi")
        ...
        request = {
            ...
            "deviation": self.slippage,
            "magic": self.magic,
            "comment": "Tortinmang PTP",
            "type_time": mt5.ORDER_TIME_GTC,
            "type_filling": filling,
        }
        result = mt5.order_send(request)
        if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
            err = str(mt5.last_error()) if result is None else f"Retcode:{result.retcode}"
            logger.error(f"❌ Partial close #{ticket}: {err}")
            return TradeResult(success=False, error=err)
        logger.info(f"✅ Partial close #{ticket}: {vol} lot yopildi")
        return TradeResult(success=True, ticket=ticket, price=price)
