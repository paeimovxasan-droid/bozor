"""
TORTINMANG.UZ — Multi-Market Signal Scanner v2
...
class MultiMarketScanner:
    ...
    async def _scan_symbol(self, symbol: str, df: pd.DataFrame, balance: float) -> Optional[MarketSignal]:
        ...
        try:
            ...
            # SIGNAL_STRENGTH (.env) bilan sozlanadi, default 14
            thr = config.risk.signal_strength
            if total >= thr:     signal = "BUY"
            elif total <= -thr:  signal = "SELL"
            else:                return None
            ...
            # MIN_ADX (.env) bilan sozlanadi, default 20
            adx = self._calc_adx(df)
            if adx < config.risk.min_adx:
                logger.debug(
                    f"ADX filter: {symbol} skip (ADX={adx:.1f} < {config.risk.min_adx}, bozor choppy)"
                )
                return None
